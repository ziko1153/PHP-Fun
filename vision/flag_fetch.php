...
$query = '';
$output = array();
$query .= "SELECT * FROM tbl_image WHERE flag_status = '1' ";
...
foreach($result as $row)
{
	...
	$sub_array[] = $image;
	$sub_array[] = $row["image"];
	$sub_array[] = '<textarea readonly>'.$row["match_key"].'</textarea>';
	$sub_array[] = '<textarea readonly>'.$row["match_pattern"].'</textarea>';
...
$output = array(
	...
	"recordsFiltered"	=>	get_total_all_falg_records(),
	"data"				=>	$data
);
